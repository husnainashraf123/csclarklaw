...
class UberSparkOptions{
	...
	function __construct( $id, $config = array() , $links = array() , $baseURL = '' ){
		...
		if( $baseURL == '' ) $baseURL = plugins_url().'/'.str_replace( basename( __FILE__ ),"",plugin_basename( __FILE__ ));
	...
	function loadCSS(){
		wp_enqueue_style('sparkoptions', 	$this->baseURL.'sparkoptions.css', 	false, '1.1', 'all');
		wp_enqueue_style('sparkoptions-icons', 	$this->baseURL.'icons/css/font-awesome.min.css', 	false, '3.0.2', 'all');
	...
	function loadJS(){
		...
		wp_enqueue_script( 'sparkoptions-js', 	$this->baseURL.'sparkoptions.js', 	false, '1.1', 'all');
			...
			<div class="spark-settings-panel">
				<div class="spark-nav">
					...
					<?php
						...
						ksort( $this->panelOrder );
						foreach($this->panelOrder as $weights ){
							foreach( $weights as $weight => $panel_id ){
								$config = $this->panels[$panel_id];
								?>
					...
					<div class="spark-nav-footer">
						<h5 style="margin-bottom:5px;">Resources</h5>
						<?php 
					...
					<form method="post" id="spark-options">
						...
						foreach($this->panels as $panel_id => $config){		
							...
							<div id="spark-<?php echo $panel_id; ?>" class="spark-panel">
								...
								<?php
								$reset = false;
								ksort( $config['ops'] );	//get the weights in the right order
								foreach( $config['ops'] as $weight ){
	...
	function showAdminOption($id, $config, $class=''){
		...
		switch($type){
			...
			case 'text':
				$html.= $title;
				$html.= '<input type="text" id="'.$id.'" name="'.$id.'" value="'.htmlspecialchars( stripslashes( $val ) , ENT_QUOTES ) .'"/>';
				$html.= $units;
				$html.= $desc;
			...
			case 'color':
				...
				$html.= '<div class="wp-colorpicker-uber-wrapper">';

				$c1_val = (string) $val;
				if( trim( $c1_val ) !== '' && strpos( $c1_val , '#' ) === false ) $c1_val = '#' . (string) $c1_val;
				
				$html.= '<input type="text" id="'.$id.'" name="'.$id.'" value="'.$c1_val.'" class="wp-colorpicker-uber" />';
				
				//$html.= '<input class="colorPicker-color" readonly="readonly" type="text" id="'.$id.'" name="'.$id.'" value="'.$val.'" />';
				
				if( $gradient ){
					$c2 = isset( $settings[$id.'-color2'] ) ? $settings[$id.'-color2'] : '';
					if( trim( $c2 ) !== '' && strpos( $c2 , '#' ) === false ) $c2 = '#' . (string) $c2;
					
					$html.= '<input type="text" id="'.$id.'-color2" name="'.$id.'-color2" value="'.$c2.'" class="wp-colorpicker-uber wp-colorpicker-uber-2" />';

					//$html.= '<input class="colorPicker-color colorPicker-color2" readonly="readonly" type="text" id="'.$id.'-color2" name="'.$id.'-color2" value="'.$c2.'" />';
				}
	 ...
	 */
	function addTextInput( $panel_id, $id, $title , $desc = '' , $default = '' , $special_class = '' , $units = '' , $weight = 10 ){
		...
		$this->addToPanel( $panel_id , $id , $weight );
	}

	function addTextArea( $panel_id, $id, $title , $desc = '' , $default = '' , $special_class = '' , $weight = 10 ){
	...
	function addCheckbox( $panel_id , $id , $title , $desc = '' , $default = 'off' , $special_class = '' , $weight = 10 ){
	...
	function addRadio( $panel_id , $id , $title , $desc = '' , $ops = array() , $default = '' , $special_class = '' , $weight = 10 ){
	...
	function addSelect( $panel_id , $id , $title , $desc = '' , $ops = array() , $default = '' , $special_class = '' , $weight = 10 ){
	...
	function addColorPicker( $panel_id , $id , $title , $desc = '' , $gradient = true , $weight = 10 ){
	...
	function addSubHeader( $panel_id, $id, $title, $desc = '', $special_class = '' , $weight = 10 ){
	 ...
	 */
	function addInfobox( $panel_id , $id , $title , $desc = '' , $special_class = '' , $weight = 10 ){
	...
	function addResetButton( $panel_id , $id , $title , $desc = '' , $special_class = '' , $weight = 10 ){
	...
	function addFlushRewriteButton( $panel_id , $id , $title , $desc = '' , $special_class = '' , $weight = 10 ){
	...
	function addCustomField( $panel_id , $id , $func , $weight = 10 ){
	...
	function addHidden( $panel_id , $id , $value , $weight = 10 ){
	...
	function addToPanel( $panel_id , $option_id , $weight = 10 ){
		...
		if( !isset( $this->panels[$panel_id]['ops'][$weight] ) ){
			$this->panels[$panel_id]['ops'][$weight] = array();
		}

		$this->panels[$panel_id]['ops'][$weight][] = $option_id;
		
	}
	
	function registerPanel( $panel_id , $name , $weight = 10 , $save_button = true ){
		
		$this->panels[$panel_id] = array();
		$this->panels[$panel_id]['name'] = $name;
		$this->panels[$panel_id]['ops'] = array();
		$this->panels[$panel_id]['save_button'] = $save_button;

		if( !isset( $this->panelOrder[$weight] ) ) $this->panelOrder[$weight] = array();
		$this->panelOrder[$weight][] = $panel_id;
		
	}

	function setPanelName( $panel_id , $name ){
		$this->panels[$panel_id]['name'] = $name;
	}
			...
			switch( $o['type'] ){
				...
				case 'color':
					...
					if( isset( $_POST[$key.'-color2'] ) ){
						$saveOps[$key.'-color2'] = trim( $_POST[$key.'-color2']  , '#' );
					}
					break;
			}
