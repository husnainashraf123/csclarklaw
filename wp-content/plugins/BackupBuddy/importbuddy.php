 ...
 *
 *
 * ImportBuddy by Dustin Bolton (dustinbolton.com) with PluginBuddy.com & iThemes.com.
 * 
 * ImportBuddy is the companion script to BackupBuddy.
 * Upload this script to your destination webserver in the root web directory.
 * If you need assistance visit http://pluginbuddy.com
 *
 * ITERATION: 128 - 10/06/2010
 * 
 * http://pluginbuddy.com
 *
 */
...
if ( version_compare( PHP_VERSION, $php_minimum ) < 0 ) {
	die( 'ERROR #9013. See <a href="http://anonym.to/?http://ithemes.com/codex/page/BackupBuddy:_Error_Codes#9013">this codex page for details</a>. Sorry! PHP version ' . $php_minimum . ' or newer is required for BackupBuddy to properly run. You are running PHP version ' . PHP_VERSION . '.' );
}
...
if ( !class_exists( 'PluginBuddyImportBuddy' ) ) {
	class PluginBuddyImportBuddy {
		...
		function alert( $message, $error = false, $error_code = '' ) {
			...
			if ( $error_code != '' ) {
				$message .= '<p><a href="http://anonym.to/?http://ithemes.com/codex/page/BackupBuddy:_Error_Codes#' . $error_code . '" target="_new"><i>BackupBuddy Error Code ' . $error_code . ' - Click for more details.</i></a></p>';
				$log_error = true;
			}
			if ( $log_error === true ) {
		...
		function view_step_1() {
			?>
			Please select the backup file you would like to import or migrate. Begin with a Full Backup then import the latest database backup if one exists.
			You will be given the ability to modify server settings on Step 4.<br /><br />
			Throughout the restore process you may hover over question marks such as <a class="ithemes_tip" title=" - This is an example help tip. Hover over these for additional help.">(?)</a> for additional help. 
			For support see our <a href="http://anonym.to/?http://ithemes.com/codex/page/BackupBuddy" target="_blank">troubleshooting resource links</a> or <a href="http://pluginbuddy.com/support/" target="_blank">support forum</a>.
			...
			if ( $handle = opendir( dirname( __FILE__ ) ) ) {
				...
				echo '</select>';
				echo '<a class="ithemes_tip" title=" - Select the backup file you would like to restore data from. This must be a valid BackupBuddy ZIP file.">(?)</a>';
				echo '<br /><br />';
				if ($count == 0) {
					echo ezimg::genImageTag('bullet_error.png');
					echo 'ERROR: Unable to find any BackupBuddy ZIP files. Upload the BackupBuddy ZIP file into the same directory as this file and keep the original file name. If you manually extracted upload the backup ZIP & select <i>Advanced Troubleshooting Options</i> & click <i>Proceed to Next Step Despite Warning</i>.';
					echo '<br /><br />';
				}
				closedir($handle);
			} else {
			...
			<div style="margin-left: 20px;">
				...
				<div id="toggle-advanced" class="toggled">
					...
					<input type="checkbox" name="skip_files" <?php if ( $count == 0 ) { echo 'checked'; } ?>/> Skip zip file extraction. <a class="ithemes_tip" title=" - Checking this box will result in importbuddy.php NOT extracting/unzipping your backup ZIP file for you.  You will need to manually extract it either on your local computer then upload it or using a server-based tool such as cPanel to extract it. This feature is useful if your host cannot extract the ZIP file for some reason or is timing out.">(?)</a><br />
					<input type="checkbox" name="skip_database" /> Skip import & migration of database. <br />
					<input type="checkbox" name="skip_htaccess" /> Skip migration of .htaccess file. <br />
					<!-- <input type="checkbox" name="merge_databases" /> Ignore existing WordPress data & merge database. <a class="ithemes_tip" title="WARNING - This may result in data conflicts, lost database data, or incomplete restores.">(?)</a><br /> -->
					...
					</select> <a class="ithemes_tip" title=" - Errors and other debugging information will be written to importbuddy.txt in the same directory as importbuddy.php.  This is useful for debugging any problems encountered during import.  Support may request this file to aid in tracking down any problems or bugs.">(?)</a>
					<!--
					<input type="checkbox" name="replace_existing_tables" onclick="
						if ( !confirm( 'WARNING! WARNING! WARNING! WARNING! WARNING! \n\n This will replace any existing WordPress installation in this database that has the same prefix. This could result in loss of posts, comments, pages, settings, and more. Verify you are using the exact database settings you want to be using. PluginBuddy & all related persons hold no responsibility for any loss of data caused by using this option. \n\n Are you sure you want to do this and potentially wipe existing data? \n\n WARNING! WARNING! WARNING! WARNING! WARNING!' ) ) {
							return false;
						} else {
							if ( !confirm( 'WARNING! WARNING! WARNING! WARNING! WARNING! \n\n You must make sure that you enter the correct database information when using this option or you may lose the wrong site or erase important data. Only use this option if you fully understand. This wiping can NOT be undone! \n\n Do you fully understand and wish to continue anyways? \n\n WARNING! WARNING! WARNING! WARNING! WARNING!' ) ) {
								return false;
							}
						}
					" /> Replace existing database tables that have the same name. <br />
					-->
					
					<?php
					// If not backup ZIP file is present then we provide a button to continue anyways.
					// TODO: Remove this since it doesnt really work?
					/*
					if ( $count == 0 ) {
						echo '<br /><input type="submit" name="submit" value="Proceed to Next Step Despite Missing Backup &raquo;" class="button-secondary" />';
					}
					*/
					?>
				</div>
		...
		function view_step_2() {
			...
			if ( ( isset( $_POST['skip_database'] ) ) && ( $_POST['skip_database'] == 'on' ) ) { $this->_options['skip_database'] = true; }
		...
		function view_step_3() {
			...
			if ( false === $this->_options['skip_files'] ) { // Option to skip all file updating / extracting.
				...
				echo '<b>Extracting files <a class="ithemes_tip" title=" - Your site files and/or database must be extracted/unzipped from the ZIP backup file for further processing. This may take some time depending on the size of your size and the speed of the server and server load. If this fails you may need to manually extract the files locally then upload them or use a server tool such as cPanel.">(?)</a> into path:</b><br /><br />' . ABSPATH . '<br /><br />';
				...
				if ( ( !file_exists( $this->_backupdata_file ) ) && ( !file_exists( ABSPATH . '/backupbuddy_dat.php' ) ) ) {
					$failed = true;
		...
		function view_step_4() {
			...
			echo '<label>New Blog URL <a class="ithemes_tip" title=" - This is the site address where you want the final site you are restoring / migrating to reside.">(?)</a>:</label> <input type="text" name="siteurl" value="'. $url .'" size="50" /><br /><br /><br />';
			
			echo '<b>mySQL Database Settings (unique per WordPress install) <a class="ithemes_tip" title=" - These settings control where your backed up database will be restored to.  If you are restoring to the same server, the settings below will import the database to your existing WordPress database location, overwriting your existing WordPress database already on the server.  If you are moving to a new host you will need to create a database to import into. The database settings MUST be unique for each WordPress installation.  If you use the same settings for multiple WordPress installations then all blog content and settings will be shared, causing conflicts!">(?)</a>:</b><br /><br />';
			...
			echo '<label>MySQL Server <a class="ithemes_tip" title=" - This is the address to the mySQL server where your database will be stored.  99% of the time this is localhost.  The location of your mySQL server will be provided to you by your host if it differs.">(?)</a>:</label> <input type="text" name="db_server" id="mysql_server" value="'.$this->_backupdata['db_server'].'" size="50" /><br />';
			echo '<label>Database Name <a class="ithemes_tip" title=" - This is the name of the database you want to import your blog into. The database user must have permissions to be able to access this database.  If you are migrating this blog to a new host you will need to create this database (ie using CPanel or phpmyadmin) and create a mysql database user with permissions.">(?)</a>:</label> <input type="text" name="db_name" id="mysql_name" value="'.$this->_backupdata['db_name'].'" size="50" /><br />';
			echo '<label>Database User <a class="ithemes_tip" title=" - This is the database user account that has permission to access the database name in the input above.  This user must be given permission to this database for the import to work.">(?)</a>:</label> <input type="text" name="db_user" id="mysql_user" value="'.$this->_backupdata['db_user'].'" size="50" /><br />';
			echo '<label>Database Pass <a class="ithemes_tip" title=" - This is the password for the database user.">(?)</a>:</label> <input type="text" name="db_password" id="mysql_password" value="'.$this->_backupdata['db_password'].'" size="50" /><br />';
			echo '<label>Database Prefix <a class="ithemes_tip" title=" - This is the prefix given to all tables in the database.  If you are cloning the site on the same server AND the same database name then you will want to change this or else the imported database will overwrite the existing tables.">(?)</a>:</label> <input type="text" name="db_prefix" id="mysql_prefix" id="mysql_prefix" value="' . $this->_backupdata['db_prefix'] . '" size="50" />';
			echo '<br />';
		...
		function view_step_5() {
			...
			if ( false === $this->_options['skip_database'] ) {
				$failed = !$this->restore_database();
			}
			
			if ( false === $failed ) {
				$failed = !$this->migrate_wp_config();
			}
		...
		function view_step_6() {
			$this->log( 'Beginning step 6 cleanup!' );
			...
			$this->log( 'Finished step 6 cleanup! Kerfuffle!' );
		}
		...
		function remove_file( $file, $description, $error_on_missing = false ) {
			...
			if ( file_exists( $file ) ) {
				chmod( $file, 0777 ); // High permissions to delete.
				echo 'Deleting ' . $description . ' ... ';
				if ( @unlink( $this->_options['file'] ) != 1 ) {
					...
					//echo $file;
					chmod( $file, 0755 ); // Restore a default permission set if deletion failed for some reason.
				} else {
					echo ' Done.<br />';
		...
		function load_backup_dat() {
			...
			$this->_backupdata = unserialize( $backupdata );
		...
		function restore_database() {
			...
			if ( !mysql_error() ) {
				echo 'Trying to create database ... Done.<br />';
				$this->log( 'Trying to create database.' );
			} else {
				$this->alert( 'Error trying to create database.' . mysql_error(), true );
			}
			...
			// Migrate URL and ABSPATH in database.
		...
		function import_sql_dump_line( $query ) {
			...
			// Run the query
			mysql_query("SET NAMES 'utf8'"); // Force UTF8
		...
		function migrate_database() {
			...
			*/
			...
			echo 'Checked ' . $count_tables_checked . ' tables & ' . $count_items_checked . ' items. ' . $count_items_changed . ' items updated. Done.<br />';
		...
		function print_html_header() {
			?><html>
				...
				<body>
					...
					if ( $this->_options['skip_database'] != false ) {
						echo 'WARNING: Debug option to skip the database is set to true. The database will not be imported or migrated.<br />';
					}
					if ( $this->_options['skip_htaccess'] != false ) {
		...
		function extract_files_highspeed() {
			...
			if ( strpos( $phpinfo['PHP Core']['disable_functions'], 'exec') != true ) { // If exec is not explicitly disabled in PHP...
				$command = 'unzip -q';
				$command .= " '$file' -d '$directory'";

				if ( file_exists( ABSPATH . '\unzip.exe' ) ) {
					$this->alert( 'Attempting to use provided unzip.exe for native Windows zip functionality.' );
			...
			if ( ( !file_exists ( ABSPATH . '/wp-content/uploads/temp_'.$this->_options['zip_id'].'/backupbuddy_dat.php' ) ) && ( !file_exists ( ABSPATH . '/backupbuddy_dat.php' ) ) ) {
		...
		function extract_files_mediumspeed() {
			...
			// Unzip reported success so make sure key files exist!
			if ( ( !file_exists ( ABSPATH . '/wp-content/uploads/temp_'.$this->_options['zip_id'].'/backupbuddy_dat.php' ) ) && ( !file_exists ( ABSPATH . '/backupbuddy_dat.php' ) ) ) {
				$this->alert( 'Mediumspeed extraction reported success; HOWEVER, key files are missing. Falling back to slower mode to try again.', true );
				return false;
			}
		...
		function extract_files_lowspeed() {
			...
			// Unzip reported success so make sure key files exist!
			if ( ( !file_exists ( ABSPATH . '/wp-content/uploads/temp_'.$this->_options['zip_id'].'/backupbuddy_dat.php' ) ) && ( !file_exists ( ABSPATH . '/backupbuddy_dat.php' ) ) ) {
				$this->alert( 'Mediumspeed extraction reported success; HOWEVER, key files are missing. No more methods to fall back to.', true );
				return false;
			}
		...
		function migrate_wp_config() {
			...
			if (file_exists(ABSPATH.'/wp-config.php')) {
				...
				echo '<br />' . ezimg::genImageTag( 'bullet_error.png' ) . 'wp-config.php not found. Perform Full Backup restore before DB only.<br />';
			}
